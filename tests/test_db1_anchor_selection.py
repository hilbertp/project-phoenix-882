...
class DB1AnchorSelectionTests(unittest.TestCase):
    def test_select_parent_anchor_uses_most_recent_eligible_opposite_pivot(self) -> None:
        ...
        terminal_extreme = Pivot(
            index=14,
            source_timestamp=(start + timedelta(hours=14)).isoformat(),
            kind=PivotKind.HIGH,
            price=130.0,
            candle_low=124.0,
            candle_high=130.0,
        ...
        pivots = [
            Pivot(3, (start + timedelta(hours=3)).isoformat(), PivotKind.LOW, 90.0, 90.0, 95.0),
            Pivot(7, (start + timedelta(hours=7)).isoformat(), PivotKind.LOW, 112.0, 112.0, 115.0),
            Pivot(9, (start + timedelta(hours=9)).isoformat(), PivotKind.LOW, 105.0, 105.0, 109.0),
            terminal_extreme,
        ]
        atr_values = [None] * 20
        atr_values[14] = 10.0
