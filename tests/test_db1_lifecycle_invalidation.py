...
class DB1LifecycleInvalidationTests(unittest.TestCase):
    def test_active_structure_is_not_replaced_until_anchor_range_is_breached(self) -> None:
        ...
        first_candidate = FibCandidate(
            parent_anchor=Pivot(4, (start + timedelta(hours=4)).isoformat(), PivotKind.LOW, 100.0, 100.0, 104.0),
            terminal_extreme=Pivot(10, (start + timedelta(hours=10)).isoformat(), PivotKind.HIGH, 130.0, 124.0, 130.0),
            direction=StructureDirection.UP,
            anchor_range_low=100.0,
            anchor_range_high=104.0,
        )
        blocked_candidate = FibCandidate(
            parent_anchor=Pivot(6, (start + timedelta(hours=6)).isoformat(), PivotKind.LOW, 101.0, 101.0, 104.0),
            terminal_extreme=Pivot(12, (start + timedelta(hours=12)).isoformat(), PivotKind.HIGH, 132.0, 126.0, 132.0),
            direction=StructureDirection.UP,
            anchor_range_low=101.0,
            anchor_range_high=104.0,
        )
        replacement_candidate = FibCandidate(
            parent_anchor=Pivot(16, (start + timedelta(hours=16)).isoformat(), PivotKind.HIGH, 120.0, 118.0, 120.0),
            terminal_extreme=Pivot(17, (start + timedelta(hours=17)).isoformat(), PivotKind.LOW, 90.0, 90.0, 94.0),
        ...
        self.assertEqual(
            structures[0].terminal_extreme_source_timestamp,
            (start + timedelta(hours=10)).isoformat(),
        )
        self.assertEqual(
            structures[0].invalidated_at_source_timestamp,
            (start + timedelta(hours=15)).isoformat(),
        )
        self.assertEqual(
            structures[1].terminal_extreme_source_timestamp,
