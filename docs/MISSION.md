...
1. **Outcomes must be human-validated before parameter sweeps mean anything.**
   ...
   trusted.
2. **Regime beats parameters.** Five months of grids: no (bars × ATR × entry)
   combination wins across months, but the Ichimoku 26-bar two-sides veto
   (`scripts/ichimoku_regime.py`) improved every single month tested
   (12-month 6c/4x: −35.8R → +3.2R). Any "most lucrative setup" claim must be
   reported with and without the veto.
