...
| **Prior fib-level bounces** (added 2026-07-03) | count of 0.618 / 0.786 / 0.882 already played out with wins before the entry fills | **Not built.** Thesis: the more levels already bounced, the LESS likely the 0.941 plays out too (the retrace has spent its buyers/sellers). Needs path tracking between terminal and fill: which levels were tagged and rejected before the entry level traded. **Founding case study** (user-spotted, data-confirmed): May-2026 6c/4x setup 4/11 (up leg 05-04→05-06, fill 9.8 days late): 0.5 bounced +1.3R, 0.618 +0.8R, 0.786 **+6.5R**, 0.882 **+3.2R** (a full TP2), then 0.941 bounced **+0.0R** — knifed straight through 1.05. Four escalating bounces, fifth level dead. Note the overlap with the max-entry-delay axis: the same setup is also the extreme delay exemplar — the two variables may be measuring one underlying phenomenon (an aged, spent retrace). |
